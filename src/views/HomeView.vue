...
const onsubmit = async () => {
  try {
    const responseWeather = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${location.value.toLocaleLowerCase()}&appid=${import.meta.env.VITE_API_KEY}&units=metric`)
    const responseForecast = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${location.value.toLocaleLowerCase()}&appid=${import.meta.env.VITE_API_KEY}&units=metric`)
        ...
        <section class="flex flex-col md:flex-row justify-between items-center">
          <section class="flex flex-col gap-4">
            <section class="flex gap-4 items-center ml-1">
              ...
              <section class="flex gap-2 items-center">
                <!-- <FewClouds /> -->
                <img class="w-14 h-14" :src="`https://openweathermap.org/img/wn/${state.weatherData.weatherObj.weather[0].icon}@2x.png`" alt="weather status icon">
          ...
          <section class="flex items-center gap-4">
            <!-- <Sun /> -->
            <img class="w-28" :src="`https://openweathermap.org/img/wn/${state.weatherData.weatherObj.weather[0].icon}@2x.png`" alt="weather status icon">
        ...
        <section class="flex flex-col md:flex-row w-full mt-10 gap-4 md:justify-between overflow-x-auto">
          ...
          <section v-for="(day, index) in state.weatherData.forecastObj.list" :key="index">
            ...
            <section class="flex justify-center items-center my-2">
              <!-- <Sun /> -->
              <img class="w-16" :src="`https://openweathermap.org/img/wn/${day.weather[0].icon}@2x.png`" alt="weather status icon">
            </section>
